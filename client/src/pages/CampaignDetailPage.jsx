...
const UpdatesTimeline = ({ updates, target, amountCollected, deadline }) => {
    ...
    const now = new Date();
    const startDate = new Date(deadline - (90 * 24 * 60 * 60 * 1000)); 
    ...
    ].filter(m => m.progressNeeded === 0 || m.progressNeeded < 100 || (m.progressNeeded >= 100));
...
const CampaignDetailPage = () => {
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            ...
            if (mockCampaign && mockCampaign.fundingBreakdown) {
                const goal = parseFloat(mockCampaign.target);
                // The breakdown now consists of percentages, so we calculate the ETH amount for display
